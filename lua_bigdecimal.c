...
static int
process_standard_div(lua_State *L, VP_HANDLE quotient, VP_HANDLE remainder) {
...
static int
process_metatable_div(lua_State *L, VP_HANDLE quotient, VP_HANDLE remainder) {
    if (remainder) {
        VpFree(&remainder);
    }
...
static int
process_metatable_mod(lua_State *L, VP_HANDLE quotient, VP_HANDLE remainder) {
    if (quotient) {
        VpFree(&quotient);
    }
...
lua_bigdecimal_digit_separation_count(lua_State *L)
{
    ...
    if (argc > 0) {
        ...
        lua_pushboolean(L, VpSetDigitSeparationCount(m) == (unsigned)m);
        return 1;
    }
...
static int
lua_create_bigdecimal(lua_State *L)
{
...
extern "C" DllExport
int luaopen_bigdecimal(lua_State *L) {
    lua_create_bigdecimal(L);
